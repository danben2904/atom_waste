...
% Action
tic
tau = min(dx^2,dy^2)/sqrt(max(max(Kx(:)),max(Ky(:))))/4.1;

for nit = 1:niter
  qx(2:end-1,:)      = -Kx(2:end-1, :) .* diff(P, 2:end-1,:)/dx % -dP/dx
  qy(:,2:end-1)      = -Ky(:, 2:end-1) .* diff(P, :,2:end-1)/dy % -dP/dy
